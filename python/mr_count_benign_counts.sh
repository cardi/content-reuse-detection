...
i=41
input=output-geo-WordCount-reduced-benign-$i
output=output-geo-WordCount-reduced-benign-counts-$i

hadoop dfs -rm -r $output

python mr_count_counts.py -r hadoop \
  ...
  --jobconf mapred.job.name="mr_count_benign_counts_py-$i" \
  --no-output \
